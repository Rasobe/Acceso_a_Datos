...
public class ConsultaSocio extends JFrame {
	...
	class OyenteBoton implements ActionListener {
		public void actionPerformed(ActionEvent event) {
			...
			if (event.getSource().equals(botonBuscar)) {
				if (cajaBuscar.getText().isEmpty()) {
					...
					if (listaSociosPorLocalidad.size() == 0) {
						...
						etiqNumSociosLista.setVisible(true);
					}
				}
			}
			...
			if (event.getSource().equals(botonAnyadir)) {
				if (cajaNombre.getText().isEmpty() && cajaEdad.getText().isEmpty() && cajaEstatura.getText().isEmpty()
					...
					if (cajaEdad.getText().matches("[\\D]+") || cajaEstatura.getText().matches("[\\D]+")) {
						JOptionPane.showMessageDialog(null, "No pueden haber letras en un campo de solo digitos.",
			...
			if (event.getSource().equals(botonEliminar)) {
				if (cajaId.getText().isEmpty()) {
					JOptionPane.showMessageDialog(null, "Debes introducir una id para eliminar (Busca a un socio por localidad).", "Error",
							JOptionPane.ERROR_MESSAGE);
				} else {
					abd.eliminarSocio(Integer.parseInt(cajaId.getText()));
					limpiarTodo();
					etiqNumSociosLista.setVisible(false);
					botonSiguiente.setEnabled(false);
					botonAnterior.setEnabled(false);
				}
			}
			...
			if (event.getSource().equals(botonActualizar)) {
				if (cajaNombre.getText().isEmpty() && cajaEdad.getText().isEmpty() && cajaEstatura.getText().isEmpty()
						&& cajaLocalidad.getText().isEmpty() && cajaId.getText().isEmpty()) {
					JOptionPane.showMessageDialog(null, "No puede haber un campo vacío. Rellena todos. (busca un socio por localidad y actualiza los campos que quieras)", "Error",
							JOptionPane.ERROR_MESSAGE);
				} else {
					if (cajaEdad.getText().matches("[\\D]+") || cajaEstatura.getText().matches("[\\D]+")) {
						JOptionPane.showMessageDialog(null, "No pueden haber letras en un campo de solo digitos.",
						...
						Socio s = new Socio(Integer.parseInt(cajaId.getText()), cajaNombre.getText(), Integer.parseInt(cajaEstatura.getText()),
								Integer.parseInt(cajaEdad.getText()), cajaLocalidad.getText());
						abd.actualizarSocio(s);
						limpiarTodo();
					}
				}
			}
